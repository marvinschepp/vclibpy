...
class BasicLMTD(HeatExchanger, abc.ABC):
    ...
    def calc_A_tc(self, lmtd, alpha_pri, alpha_sec, Q, state):

        k = self.calc_k_gnielinski(alpha_pri=alpha_pri, alpha_sec=alpha_sec, state=state)
    ...
    def calc_k_gnielinski(self, alpha_pri: float, alpha_sec: float, state) -> float:
        """
        Calculate the overall heat transfer coefficient (k) of the heat exchanger.
        ...
        Returns:
            float: Overall heat transfer coefficient (k).
        """
        RP = ct.REFPROPFunctionLibrary(r'C:\Users\Chara\Desktop\crispy\REFPROP')
        RP.SETPATHdll(r'C:\Users\Chara\Desktop\crispy\REFPROP')
        MASS_BASE_SI = RP.GETENUMdll(0, "MASS BASE SI").iEnum

        def get_co2_properties(T, P):
            RP.SETFLUIDSdll("CO2")
            data = RP.REFPROPdll('CO2', "PT", "D;CP;CV;VIS;TCX", MASS_BASE_SI, 0, 0, P, T, [1.0])
            density = data.Output[0]
            cp = data.Output[1]
            cv = data.Output[2]
            viscosity = data.Output[3]
            thermal_conductivity = data.Output[4]
            return density, cp, cv, thermal_conductivity, viscosity
        ...
        def gnielinski_correlation(Re, Pr, f):
            return ((f / 8) * (Re - 1000) * Pr) / (1 + 12.7 * (f / 8) ** 0.5 * (Pr ** (2 / 3) - 1))

        def friction_factor(Re):
            return (0.79 * math.log(Re) - 1.64) ** (-2)

        def calculate_nusselt(Re, Pr):
            f = friction_factor(Re)
            return gnielinski_correlation(Re, Pr, f)

        def calculate_heat_transfer_coefficient(Nu, k, D):
            return Nu * k / D


        diameter = 0.007
        density, cp, cv, thermal_conductivity, viscosity = get_co2_properties(state.T, state.p)
        velocity = self.m_flow / density / (math.pi * diameter ** 2)

        Re = calculate_reynolds(density, velocity, diameter, viscosity)
        Pr = cp * viscosity / thermal_conductivity
        Nu = calculate_nusselt(Re, Pr)
        htc = calculate_heat_transfer_coefficient(Nu, thermal_conductivity, diameter)


        k_wall = self.calc_wall_heat_transfer()
        k = (1 / (
                (1 / htc) * self.ratio_outer_to_inner_area +
                (1 / k_wall) * self.ratio_outer_to_inner_area +
                (1 / alpha_sec)
        )
             )
...
class MVBLMTDSensibleSec(BasicLMTD, abc.ABC):
    ...
    def separate_phases_tc(self, state_max: ThermodynamicState, state_min: ThermodynamicState, p: float):
        """
        ...
        states = []
        ref_temp = []
        enthalpy_dist = np.linspace(state_min.h, state_max.h, 100)
        states.append(self.med_prop.calc_state("PH", p, enthalpy_dist[0]))

        for i in range(1, n_segments):
            Qs.append(abs(enthalpy_dist[i-1] - enthalpy_dist[i])  * self.m_flow)
            states.append(self.med_prop.calc_state("PH", p, enthalpy_dist[i]))

        pinch_point_segment = None
        pinch_point_temp_diff = np.inf

        sec_temp = np.linspace(291.15, 296.15, n_segments)

        for i in range(n_segments):
            tempf_diff = states[i].T - sec_temp[i]
            if tempf_diff < pinch_point_temp_diff:
                pinch_point_temp_diff = tempf_diff
                pinch_point_segment = i
...
class MVBLMTDSensibleSecGasTc(MVBLMTDSensibleSec):
    """
    Condenser class which implements the actual `calc` method.
    ...
    def calc(self, inputs: Inputs, fs_state: FlowsheetState) -> (float, float):
        ...
        Q_i, state_i = self.separate_phases_tc(
                                                           self.state_inlet,
                                                           self.state_outlet,
                                                           self.state_inlet.p
                                                           )
        ...
        # Q = self.state_inlet.h - self.state_outlet.h
        ...
        # Assumption loss is the same correlation for each regime
        T_sec_i = []
        T_sec_i.append(inputs.T_con_in)
        A_i = []
        dT_min = []
        dT_min.append(state_i[0].T - inputs.T_con_in)

        for i in range(1, n_segments):

            T_sec_i.append(T_sec_i[i-1] + self.calc_secondary_Q_flow(Q_i[i-1]) / self.m_flow_secondary_cp)

            tra_prop_ref_con = self.med_prop.calc_mean_transport_properties(state_i[i-1], state_i[i])
            alpha_ref_wall = self.calc_alpha_gas(tra_prop_ref_con)

            lmtd = self.calc_lmtd(Tprim_in=state_i[i-1].T,
                                  Tprim_out=state_i[i].T,
                                  Tsec_in=T_sec_i[i-1],
                                  Tsec_out=T_sec_i[i])

            A_i.append(self.calc_A_tc(lmtd=lmtd,
                                      alpha_pri=alpha_ref_wall,
                                      alpha_sec=alpha_med_wall,
                                      Q=Q_i[i-1],
                                      state=state_i[i])
            ...
            dT_min.append(state_i[i].T - T_sec_i[i])
        ...
        fs_state.set(name="A_con_total", value=np.sum(A_i), unit="m2",
                     description="Area for superheat heat exchange in condenser")
        fs_state.set(name="dT_pinch_con",
                     value=min(dT_min),
                     description="Pinch Condenser")

        return error, min(dT_min)
